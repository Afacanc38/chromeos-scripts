...
zenity --question --cancel-label="İptal Et" --ok-label="Devam Et" --title="ChromeOS Yükleyici" \
          ...
          --text="Devam et tuşuna bastığınızda ChromeOS cihazınıza yüklenecektir. \n\nBu işlem bilgisayarınızdaki tüm verileri sileceğinden (İşletim sistemi dahil) önemli verilerinizi yedeklemenizi öneririz. Devam etmek istiyor musunuz?" 2>/dev/null
if [[ "$?" != "0" ]]; then ${opt_procedure[@]}
	...
	sudo bash chromeos-install.sh -src chromeos.bin -dst /dev/sda | zenity --progress \
